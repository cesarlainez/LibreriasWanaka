...
/// Versión .NET Framework 4.8: las expresiones regulares usan campos
/// <c>static readonly Regex</c> con <c>RegexOptions.Compiled</c> (el atributo
/// <c>[GeneratedRegex]</c> solo existe en .NET 7+).
/// Ejemplo:
/// <code>
...
/// </code>
/// </remarks>
public sealed class DocumentToMarkdownCleaner
{
    /// <summary>
    ...
    // ---------------------------------------------------------------------
    // Expresiones regulares compiladas (equivalente a [GeneratedRegex] en .NET 7+).
    // ---------------------------------------------------------------------
    ...
    /// </summary>
    private static readonly Regex PaginationLineRegex = new Regex(
    ...
    /// </summary>
    private static readonly Regex PrintDateNoiseRegex = new Regex(
    ...
    private static readonly Regex CaratulaNoiseRegex = new Regex(
        @"\bcar[áa]tula\b",
        RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);

    /// <summary>Comentario HTML vacío: <c>&lt;!-- --&gt;</c> (solo espacios/saltos dentro).</summary>
    private static readonly Regex EmptyHtmlCommentRegex = new Regex(
        @"<!--\s*-->",
    ...
    /// </summary>
    private static readonly Regex LongUnderscoreRunRegex = new Regex(
        @"_{6,}",
        RegexOptions.CultureInvariant | RegexOptions.Compiled);

    /// <summary>Línea compuesta únicamente por espacios o tabulaciones.</summary>
    private static readonly Regex WhitespaceOnlyLineRegex = new Regex(
        @"^[ \t]+$",
